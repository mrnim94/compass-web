...
/**
 * @typedef {{progressCallback: ExportCallbackFunc?, finishCallback: ExportCallbackFunc?}} ExportCallbacks
 */

/**
 * @callback ExportCallbackFunc
 * @param {number} currentDocs
 */
...
function exportJSON(cursor, callbacks, variant) {
  ...
  const docStream = new Transform({
    ...
    transform: function (chunk, encoding, callback) {
      ...
      ++docsWritten;
      callbacks.progressCallback?.(docsWritten);
    ...
    final: function (callback) {
      ...
      this.push(']');
      callbacks.finishCallback?.(docsWritten);
      callback(null);
    },
  });
 ...
 */
async function exportCSV(cursor, callbacks, delimiter) {
  // Get column names
...
function exportJSONFromAggregation(
  ...
  return exportJSON(cursor, callbacks, variant);
}
 ...
 *
 */
function exportJSONFromQuery(ns, query, callbacks, variant, client) {
 ...
 *
 */
async function exportCSVFromQuery(ns, query, delimiter, callbacks, client) {
  const cursor = createFindCursor(ns, query, client);

  return exportCSV(cursor, callbacks, delimiter);
}

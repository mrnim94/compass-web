...
describe('importJSON', () => {
  ...
  for (const fixtureType of ['json', 'jsonl']) {
    for (const filepath of Object.values(fixtures[fixtureType])) {
      ...
      test(`imports ${basename}`, async function () {
  ...
  test('imports a file containing multiple batches', async function () {
    const lines = [];
  ...
  test('errors if a json file contains things that are not arrays', async function () {
    const ns = 'db.col';
  ...
  test('errors if a jsonl file contains things that are not objects', async function () {
    const ns = 'db.col';
  ...
  test('errors if there are database errors (stopOnErrors=true)', async function () {
  ...
  test('reports and writes database errors (stopOnErrors=false)', async function () {
    const lines = [{ i: 0 }, { i: 1 }].map((doc) => JSON.stringify(doc));

...
describe('exportJSON', () => {
  ...
  for (const fixtureType of ['json', 'jsonl']) {
    for (const filepath of Object.values(fixtures[fixtureType])) {
      ...
      test(`exports ${basename}`, async () => {
        ...
        const docsExported = progressCallback.mock.calls.length;
        expect(docsExported).toBeGreaterThan(0);

        const docs = await coll.find({}).toArray();
        expect(docs).toHaveLength(docsExported);
  ...
  test('exports aggregations', async () => {
...
describe('exportCSV', () => {
  ...
  test('exports an empty collection', async () => {
    let docsWritten = 0;
    let textResult = await text(
      await exportCSVFromQuery(
        testNS,
        { filter: {} },
        ',',
        {
    ...
    textResult = replaceId(textResult);
